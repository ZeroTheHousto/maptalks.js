...
const Canvas = {
  ...
  _textOnMultiRow (ctx, texts, style, point, splitTextSize, textSize) {
    ...
    const maxHeight = style.textMaxHeight
    let text; let rowAlign; let height = 0
  ...
  polygon (ctx, points, lineOpacity, fillOpacity, lineDashArray, smoothness) {
    ...
    const fillFirst = (isArrayHasData(lineDashArray) && !ctx.setLineDash) || isPatternLine && !smoothness
  ...
  paintSmoothLine (ctx, points, lineOpacity, smoothValue, close, tailIdx, tailRatio) {
    ...
    function getCubicControlPoints (x0, y0, x1, y1, x2, y2, x3, y3, smoothValue, t) {
      ...
      //      x3,y3 - the next one.
      const xc1 = (x0 + x1) / 2.0; const yc1 = (y0 + y1) / 2.0
      const xc2 = (x1 + x2) / 2.0; const yc2 = (y1 + y2) / 2.0
      const xc3 = (x2 + x3) / 2.0; const yc3 = (y2 + y3) / 2.0
      ...
      const xm1 = xc1 + (xc2 - xc1) * k1; const ym1 = yc1 + (yc2 - yc1) * k1

      const xm2 = xc2 + (xc3 - xc2) * k2; const ym2 = yc2 + (yc3 - yc2) * k2
