...
class Circle extends CenterMixin(Polygon) {
  ...
  _containsPoint (point, tolerance) {
    ...
    const se = center.add(size.width / 2, size.height / 2)
  ...
  _computePrjExtent (projection) {
    ...
    const dy = Math.min(Math.abs(pminmax[2].y - pcenter.y), Math.abs(pminmax[3].y - pcenter.y))
  ...
  _getMinMax (measurer) {
    ...
    const p2 = measurer.locate(this._coordinates, radius, 0)
    const p3 = measurer.locate(this._coordinates, 0, radius)
    const p4 = measurer.locate(this._coordinates, 0, -radius)
    return [p1, p2, p3, p4]
  }
