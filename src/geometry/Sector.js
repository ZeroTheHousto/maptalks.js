...
class Sector extends Circle {
  ...
  getShell () {
    ...
    const numberOfPoints = this.options.numberOfShellPoints - 2
    const radius = this.getRadius()
    const shell = [center.copy()]
    const startAngle = this.getStartAngle()
    const angle = this.getEndAngle() - startAngle
    let rad, dx, dy
  ...
  _containsPoint (point, tolerance) {
    ...
    // [0.0, 360.0)
    const angle = atan2 < 0 ? (atan2 + 2 * Math.PI) * 360 / (2 * Math.PI)
      : atan2 * 360 / (2 * Math.PI)
    const sAngle = this.startAngle % 360
    const eAngle = this.endAngle % 360
    let between = false
    if (sAngle > eAngle) {
