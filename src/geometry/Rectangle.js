...
class Rectangle extends Polygon {
  ...
  _getPrjShell () {
    ...
    const sx = sphereExtent.sx
    const sy = sphereExtent.sy
  ...
  _getSouthEast (measurer) {
    ...
    if (measurer.fullExtent) {
      ...
      const sx = fullExtent.right > fullExtent.left ? 1 : -1
      const sy = fullExtent.top > fullExtent.bottom ? 1 : -1
      w *= sx
      h *= sy
    }
