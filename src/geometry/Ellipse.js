...
class Ellipse extends CenterMixin(Polygon) {
  ...
  getShell () {
    ...
    const numberOfPoints = this.options.numberOfShellPoints
    const width = this.getWidth()
    const height = this.getHeight()
    const shell = []
    const s = Math.pow(width / 2, 2) * Math.pow(height / 2, 2)
    const sx = Math.pow(width / 2, 2)
    const sy = Math.pow(height / 2, 2)
    let deg, rad, dx, dy
  ...
  _containsPoint (point, tolerance) {
    ...
    const pps = projection.projectCoords([this._coordinates, map.locate(this._coordinates, this.getWidth() / 2, this.getHeight() / 2)])
    const p0 = map._prjToContainerPoint(pps[0])
    const p1 = map._prjToContainerPoint(pps[1])
  ...
  _getMinMax (measurer) {
    ...
    const p2 = measurer.locate(this._coordinates, width / 2, 0)
    const p3 = measurer.locate(this._coordinates, 0, -height / 2)
    const p4 = measurer.locate(this._coordinates, 0, height / 2)
    return [p1, p2, p3, p4]
  }
