...
class Path extends Geometry {
  ...
  _drawAnimShowFrame (t, duration, length, coordinates) {
    ...
    const p2 = coordinates[idx + 1]
    const span = targetLength - this._animLenSoFar
    const r = span / segLen
    this._animTailRatio = r
    const x = p1.x + (p2.x - p1.x) * r
    const y = p1.y + (p2.y - p1.y) * r
    const targetCoord = new Coordinate(x, y)
    const isPolygon = !!this.getShell
  ...
  _getPath2DPoints (prjCoords, disableSimplify, zoom) {
    ...
    const map = this.getMap()
    const isSimplify = !disableSimplify && this._shouldSimplify()
    const tolerance = this.options.simplifyTolerance * map._getResolution()
    const isMulti = Array.isArray(prjCoords[0])
