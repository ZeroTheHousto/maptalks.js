...
class TileLayer extends Layer {
  ...
  _getTiles (z, containerExtent, maskID, parentRenderer) {
    ...
    const left = Math.ceil(Math.abs(centerTile.x - ltTile.x))
    const bottom = Math.ceil(Math.abs(centerTile.y - rbTile.y))
    const right = Math.ceil(Math.abs(centerTile.x - rbTile.x))
    const tileSize = this.getTileSize()
    const layerId = this.getId()
    const renderer = this.getRenderer() || parentRenderer
    const scale = this._getTileConfig().tileSystem.scale
    const visited = {}
    const tiles = []; const extent = new PointExtent()
    for (let i = -top; i <= bottom; i++) {
      ...
      while (j >= leftVisitEnd && j <= right) {
        ...
        if (sr === mapSR) {
          ...
          const pp = map._prjToPoint(this._unproject(pse, TEMP_POINT3), z, TEMP_POINT3)
          width = Math.ceil(Math.abs(pp.x - p.x))
          height = Math.ceil(Math.abs(pp.y - p.y))
        }
        const dx = scale.x * (idx.idx - idx.x) * width
        const dy = -scale.y * (idx.idy - idx.y) * height
        if (dx || dy) {
          p._add(dx, dy)
        }
        if (hasOffset) {
  ...
  _getInnerExtent (zoom, containerExtent, extent2d) {
    ...
    const center = extent2d.getCenter()._multi(scale)
    const bearing = map.getBearing() * Math.PI / 180
    const ch = containerExtent.getHeight() / 2 * scale
    const cw = containerExtent.getWidth() / 2 * scale
    const h = Math.abs(Math.cos(bearing) * ch) || ch
    const w = Math.abs(Math.sin(bearing) * ch) || cw
