...
class Zoom extends Control {
  ...
  _onClickRuler (e) {
    preventDefault(e)
    const map = this.getMap()
    const point = getEventContainerPoint(e, this._sliderRuler)
    const h = point.y
    const maxZoom = map.getMaxZoom()
    const zoom = Math.floor(maxZoom - h / UNIT)
    map.setZoom(zoom)
  }
  ...
  _onDotDragstart (e) {
    ...
    const map = this.getMap()
    const origin = map.getSize().toPoint()._multi(1 / 2)
  ...
  _onDotDrag (e) {
    ...
    const map = this.getMap()
    const origin = map.getSize().toPoint()._multi(1 / 2)
    const point = getEventContainerPoint(e.domEvent, this._sliderRuler)
    const maxZoom = map.getMaxZoom()
    const minZoom = map.getMinZoom()
    let top = point.y
    let z = maxZoom - top / UNIT
