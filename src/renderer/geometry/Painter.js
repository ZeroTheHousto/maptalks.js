...
class Painter extends Class {
  ...
  _createSymbolizers () {
    ...
    const symbolizers = []
    const regSymbolizers = registerSymbolizers
  ...
  _meterToPoint (center, altitude) {
    ...
    const p1 = map.coordToPoint(target, z, TEMP_POINT2)
    return Math.abs(p1.x - p0.x) * sign(altitude)
  }

  _getAltitudeProperty () {
    const geometry = this.geometry
    const layerOpts = geometry.getLayer().options
