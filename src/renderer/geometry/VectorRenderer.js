...
const el = {
  ...
  _getRenderSize () {
    ...
    const pmax = map._prjToPoint(prjExtent.getMax(), z)
...
LineString.include({
  ...
  _getArrowShape (prePoint, point, lineWidth, arrowStyle, tolerance) {
    ...
    const height = lineWidth * arrowStyle[1] + tolerance
    const hw = width / 2 + tolerance
  ...
  _getArrows (points, lineWidth, tolerance) {
    ...
    const arrows = []
    const map = this.getMap()
    const first = map.coordToContainerPoint(this.getFirstCoordinate())
    const last = map.coordToContainerPoint(this.getLastCoordinate())
