...
const Map = (container, options) => {
   ...
   * @protected
   */
  const addOnLoadHook = (fn) => { // (Function) || (String, args...)
   ...
   * @return {Boolean}
   */
  const isLoaded = () => !!this._loaded

  /**
   * Get map's container
   * @returns {HTMLElement}
   */
  const getContainer = () => this._containerDOM

  /**
     * Get the spatial reference of the Map.
     * @return {SpatialReference} map's spatial reference
     */
  const getSpatialReference = () => this._spatialReference
  ...
  const _updateSpatialReference = (ref, oldRef) => {
    ref = extend({}, ref)
   ...
   * map.resetCursor();
   */
  const resetCursor = () => this.setCursor(null)
  ...
  const _getVisualHeight = (maxVisualPitch) => {
    const pitch = this.getPitch()
  ...
  /**
     ...
     * @return {Extent}
     */
  const getExtent = () => this._pointToExtent(this._get2DExtent())
  ...
  /**
     ...
     * @return {Extent}
     */
  const getPrjExtent = () => this.getProjExtent()
   ...
   */
  const getZoomForScale = (scale, fromZoom, isFraction) => {
  ...
  const getZoomFromRes = res => {
    const resolutions = this._getResolutions()
    const minRes = this._getResolution(this.getMinZoom())
    const maxRes = this._getResolution(this.getMaxZoom())
    if (minRes <= maxRes) {
      if (res <= minRes) {
   ...
   * @returns {Map} this
   */
  const setZoom = (zoom, options = { animation: true }) => {
   ...
   * @returns {Number}
   */
  const getGLZoom = () => this.getMaxNativeZoom() / 2
  ...
  /**
     ...
     * @return {Map} this
     */
  const zoomIn = () => this.setZoom(this.getZoom() + 1)

  /**
     * zoom out
     * @return {Map} this
     */
  const zoomOut = () => this.setZoom(this.getZoom() - 1)
  ...
  /**
     ...
     * @return {Boolean}
     */
  const isInteracting = () => this.isZooming() || this.isMoving() || this.isRotating()
  ...
  /**
   * Calculate the zoom level that contains the given extent with the maximum zoom level possible.
   * @param {Extent} extent
   * @return {Number} zoom fit for the extent
   */
  const getFitZoom = extent => {
    ...
    const h = containerExtent.getHeight()
  ...
  /**
     ...
     */
  const getResolution = zoom => this._getResolution(zoom)
  ...
  const getScale = zoom => {
    ...
    const res = this._getResolution(z)
    return res / max
  }
  ...
  /**
     ...
     * @return {Map} - this
     */
  const fitExtent = (extent, zoomOffset, options = {}, step) => {
    if (!extent) {
      return this
    }
    ...
    } else { return this.setCenterAndZoom(center, zoom) }
  }
  ...
  /**
     ...
     * @return {Layer}
     */
  const getBaseLayer = () => this._baseLayer
   ...
   */
  const coordToPoint = (coordinate, zoom, out) => this.coordinateToPoint(coordinate, zoom, out)

  /**
     * shorter alias for pointToCoordinate
     */
  const pointToCoord = (point, zoom, out) => this.pointToCoordinate(point, zoom, out)

  /**
   * shorter alias for coordinateToViewPoint
   */
  const coordToViewPoint = (coordinate, out) => this.coordinateToViewPoint(coordinate, out)

  /**
     * shorter alias for viewPointToCoordinate
     */
  const viewPointToCoord = (viewPoint, out) => this.viewPointToCoordinate(viewPoint, out)

  /**
     * shorter alias for coordinateToContainerPoint
     */
  const coordToContainerPoint = (coordinate, zoom, out) => this.coordinateToContainerPoint(coordinate, zoom, out)

  /**
     * shorter alias for containerPointToCoordinate
     */
  const containerPointToCoord = (containerPoint, out) => this.containerPointToCoordinate(containerPoint, out)
  ...
  /**
     ...
     * @returns {Point}
     */
  const containerPointToViewPoint = (containerPoint, out) => {
    if (out) {
  ...
  /**
     ...
     * @returns {Point}
     */
  const viewPointToContainerPoint = (viewPoint, out) => {
    if (out) {
  ...
  /**
     ...
     */
  const locate = (coordinate, dx, dy) => this.getProjection()._locate(new Coordinate(coordinate), dx, dy)

  /**
     * Return map's main panel
     * @returns {HTMLElement}
     */
  const getMainPanel = () => this._getRenderer().getMainPanel()

  /**
     * Returns map panels.
     * @return {Object}
     */
  const getPanels = () => this._panels
  ...
  /**
     ...
     * @return {Boolean}
     */
  const isRemoved = () => !this._containerDOM
  ...
  const isDragRotating = () => !!this._dragRotating
  ...
  /**
     ...
     * @returns {Boolean}
     */
  const isOffscreen = (box, viewportPadding = 0) => {
  ...
  const getRenderer = () => this._getRenderer()

  /**
     * Get device's devicePixelRatio, you can override it by setting devicePixelRatio in options.
     * @returns {Number}
     */
  const getDevicePixelRatio = () => this.options.devicePixelRatio || Math.ceil(Browser.devicePixelRatio) || 1

  // -----------------------------------------------------------

  const _initContainer = container => {
  ...
  const _setPriorityCursor = cursor => {
    if (!cursor) {
      let hasCursor = false
  ...
  const _setCursorToPanel = cursor => {
  ...
  const _removeLayer = (layer, layerList) => {
    if (!layer || !layerList) {
      return
    }
  ...
  const _fireEvent = (eventName, param) => {
    if (this._noEvent) {
      return
    }
   ...
   * @private
   */
  const _getPrjCenter = () => this._prjCenter

  const _setPrjCenter = pcenter => {
  ...
  const _setPrjCoordAtContainerPoint = (coordinate, point) => {
  ...
  /**
     ...
     */
  const _offsetCenterByPixel = (pixel) => {
  ...
  const _getResolutions = () => this._spatialReference.getResolutions()
   ...
   * @private
   */
  const _prjToPoint = (pCoord, zoom, out) => {
   ...
   * @private
   */
  const _pointToPrj = (point, zoom, out) => {
    zoom = (isNil(zoom) ? this.getZoom() : zoom)
   ...
   * @private
   */
  const _pointToPoint = (point, zoom, out) => {
    if (out) {
      out.x = point.x
      out.y = point.y
    } else {
   ...
   * @private
   */
  const _pointToPointAtZoom = (point, zoom, out) => {
    if (out) {
      out.x = point.x
      out.y = point.y
    } else {
   ...
   * @private
   */
  const _containerPointToPrj = (containerPoint, out) =>
    this._pointToPrj(this._containerPointToPoint(containerPoint, undefined, out), undefined, out)

  const _callOnLoadHooks = () => {
  ...
  distanceToPixel: (function () {
    ...
    return function (xDist, yDist, zoom) {
      ...
      const center = this.getCenter()
      const target = projection.locate(center, xDist, yDist)
      const p0 = this.coordToContainerPoint(center, undefined, POINT0)
      const p1 = this.coordToContainerPoint(target, undefined, POINT1)
  ...
  distanceToPoint: (function () {
    ...
    return function (xDist, yDist, zoom) {
      ...
      const p1 = this.coordToPoint(target, zoom)
      p1._sub(p0)._abs()
      return p1
    }
  }()),
