...
export default class SpatialReference {
  ...
  static equals (sp1, sp2) {
    ...
    const f1 = sp1.fullExtent; const f2 = sp2.fullExtent
    if (f1 && !f2 || !f1 && f2) {
      return false
    }
    if (f1 && f2) {
    ...
    const r1 = sp1.resolutions; const r2 = sp2.resolutions
    if (r1 && r2) {
  ...
  _initSpatialRef () {
    ...
    let resolutions = this.options.resolutions
    if (!resolutions) {
      if (projection.code) {
    ...
    const b = fullExtent.top >= fullExtent.bottom ? -1 : 1
