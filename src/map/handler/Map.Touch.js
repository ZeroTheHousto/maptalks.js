...
class MapTouchZoomHandler extends Handler {
  ...
  _onTouchStart (event) {
    ...
    const p2 = getEventContainerPoint(event.touches[1], container)
  ...
  _onTouchMove (event) {
    ...
    const p1 = getEventContainerPoint(event.touches[0], container)
    const p2 = getEventContainerPoint(event.touches[1], container)
    const d1 = p1.sub(this._startP1)
    const d2 = p2.sub(this._startP2)
    const vector = p1.sub(p2)
    const scale = p1.distanceTo(p2) / this._startDist
    const bearing = vector.angleWith(this._startVector) * 180 / Math.PI
    const preY = this.preY || p1.y
    const pitch = (preY - p1.y) * 0.4
