...
Map.include(/** @lends Map.prototype */{
  ...
  _calcFrustumAltitude () {
    ...
    const d2 = cameraAlt * Math.tan(fov * 2)
  ...
  _pointToContainerPoint: (function () {
    ...
    return function (point, zoom, altitude = 0, out) {
      ...
      if (this.isTransforming() || altitude) {
        ...
        const w2 = this.width / 2; const h2 = this.height / 2
        t[0] = (t[0] * w2) + w2
        t[1] = -(t[1] * h2) + h2
        if (out) {
          out.x = t[0]
  ...
  _containerPointToPoint: (function () {
    ...
    const coord1 = [0, 0, 0, 1]
    return function (p, zoom, out) {
      if (this.isTransforming()) {
        const w2 = this.width / 2 || 1; const h2 = this.height / 2 || 1
      ...
      const scale = (zoom !== undefined ? this._getResolution() / this._getResolution(zoom) : 1)
      const x = scale * (p.x - this.width / 2)
      const y = scale * (p.y - this.height / 2)
  ...
  _calcMatrices: (function () {
    ...
    const m1 = Browser.ie9 ? null : createMat4()
    return function () {
