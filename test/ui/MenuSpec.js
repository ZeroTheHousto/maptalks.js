describe('UI.ContextMenu', function () {
  ...
  it('hide when geometry is hided', function () {
    ...
    const target = new maptalks.Marker(map.getCenter())
  ...
  it('move when geometry is moved', function () {
    ...
    target.openMenu()
    const w = target._menu
    const pos1 = w.getPosition()

    target.setCoordinates(map.getCenter().add(0.1, 0.1))

    const pos2 = w.getPosition()
  ...
  it('specify menu height', function () {
    ...
    target.openMenu()
    const menuDom = target._menu.getDOM().firstChild
...
function runTests (target, _context) {
  ...
  function assertItems () {
    const itemEles = document.getElementsByTagName('li')
  ...
  function rightclick () {
    ...
    const eventContainer = _context.map._panels.canvasContainer
    const domPosition = GET_PAGE_POSITION(eventContainer)
    const point = _context.map.coordinateToContainerPoint(target.getFirstCoordinate()).add(domPosition)
  ...
  context('Type of ' + type, function () {
    ...
    it('setMenu and open with animation', function (done) {
      ...
      const p = target.getMap().coordinateToViewPoint(target.getCenter())._round()
    ...
    it('callback will be called when item is clicked', function () {
      ...
      target.openMenu()
      let itemEles = document.getElementsByTagName('li')
      itemEles[0].click()
