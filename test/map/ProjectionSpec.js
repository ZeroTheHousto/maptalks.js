describe('Map.Projection', function () {
  ...
  it('customize projection', function () {
    ...
    const actual = maptalks.Coordinate.toNumberArrays(projection.projectCoords(coords))
  ...
  describe('change to EPSG:4326', function () {
    ...
    it('change center before changing spatial reference', function () {
      const newCenter = new maptalks.Coordinate(100, 0)
  ...
  describe('change to IDENTITY', function () {
    it('change to IDENTITY', function () {
      ...
      const circle = new maptalks.Circle([10, 10], 1)
    ...
    it('fit to extent in IDENTITY projection', function () {
      ...
      const z = map.getFitZoom(map.getMaxExtent())
    ...
    it('rotate polygon with  IDENTITY projection, #726', function () {
      ...
      const vecLayer = new maptalks.VectorLayer('field').addTo(map)
      const baseGeom = new maptalks.Polygon([[[0, 0], [0, 20], [20, 30], [20, 0]]], {
        symbol: {
          lineWidth: 2,
          lineColor: '#000',
      ...
      const geojson = { type: 'Feature', geometry: { type: 'Polygon', coordinates: [[[0, 0], [0, 20], [20, 30], [20, 0], [0, 0]]] }, properties: null }
  ...
  describe('change to Baidu', function () {
    ...
    it('baidu projection with Infinity', function () {
      const baiduProj = maptalks.SpatialReference.getProjectionInstance('baidu')
      const a = baiduProj.project(new maptalks.Coordinate(Infinity, -Infinity))
      const b = baiduProj.project(new maptalks.Coordinate(-Infinity, -Infinity))
    })
  })
})
