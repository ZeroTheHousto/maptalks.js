...
describe('Map.Camera', function () {
  ...
  describe('TileLayer\'s rendering', function () {
    it('gl render after composite operations', function (done) {
      ...
      const baseLayer = new maptalks.TileLayer('b', {
  ...
  describe('conversions', function () {
    ...
    context('point to containerPoint', function () {
      it('1', function () {
        ...
        const point = map2.coordToPoint(center)
        const cp = map2._pointToContainerPoint(point)
        expect(cp).to.be.closeTo(map2.getSize().toPoint().multi(1 / 2))
        const point2 = map2._containerPointToPoint(cp)
      ...
      it('2.001', function () {
        const ncenter = map.containerPointToCoord({ x: 15, y: 15 })
      ...
      it('2.1', function () {
        ...
        const point = map.coordToPoint({ x: extent.xmin, y: extent.ymax })
        const cp = map._pointToContainerPoint(point)

        expect(cp.x).to.be.approx(0)
        expect(cp.y).to.be.approx(0)

        const coord = map.containerPointToCoord(cp)
      ...
      it('2.2', function () {
        map.setPitch(1)
        const extent = map.getExtent()
        const point = map.coordToPoint({ x: extent.xmax, y: extent.ymax })
        const cp = map._pointToContainerPoint(point)
      ...
      it('2.3', function () {
        map.setPitch(1)
        const extent = map.getExtent()
        const point = map.coordToPoint({ x: extent.xmin, y: extent.ymin })
        const cp = map._pointToContainerPoint(point)
      ...
      it('2.4', function () {
        map.setPitch(1)
        const extent = map.getExtent()
        const point = map.coordToPoint({ x: extent.xmax, y: extent.ymin })
        const cp = map._pointToContainerPoint(point)
    ...
    it('containerPoint and viewPoint', function () {
      ...
      let cp = map.coordinateToContainerPoint(center)
      let vp = map.containerPointToViewPoint(cp)
      let cp2 = map.viewPointToContainerPoint(vp)
    ...
    it('pixel and distance', function () {
      let size = map.distanceToPixel(300, 400)
      let dist = map.pixelToDistance(size.width, size.height)
    ...
    it('containerPoint and coordinate', function () {
      const center = map.getCenter()
      let cp = map.coordinateToContainerPoint(center)
      let c2 = map.containerPointToCoordinate(cp)
    ...
    it('containerPoint and point', function () {
      map.setPitch(30)
      const point = map._containerPointToPoint({ x: 0, y: 0 })
      const pointMax = map._containerPointToPoint({ x: 0, y: 0 }, map.getMaxZoom())
  ...
  describe('polygon\' size when pitching or rotating', function () {
    it('changed when rotating', function () {
      const top = new maptalks.Coordinate([center.x, center.y + 0.001])
  ...
  describe('marker\' size should be unchanged when pitching or rotating', function () {
    it('image marker', function () {
      const marker = new maptalks.Marker([100, 0], {
        symbol: {
      ...
      const s = new maptalks.Point(20, 30)
    ...
    it('vector marker', function () {
      ...
      const s = new maptalks.Point(21, 31)
      expect(marker.getSize().toPoint()).to.be.eql(s)
      map.setPitch(60)
      expect(marker.getSize().toPoint()).to.be.eql(s)
      map.setBearing(40)
    ...
    it('vector path marker', function () {
      ...
      const marker = new maptalks.Marker([100, 0]).addTo(layer)
      const s = new maptalks.Point(24, 34)
      expect(marker.getSize().toPoint()).to.be.eql(s)
      map.setPitch(60)
      expect(marker.getSize().toPoint()).to.be.closeTo(s)
      map.setBearing(40)
  ...
  describe('draw with altitude when pitching', function () {
    it('circle', function (done) {
      const geometry = new maptalks.Circle(center, 5, {
        properties: {
          altitude: 30
        },
        symbol: {
  ...
  describe('containsPoint when pitching', function () {
    it('linestring', function () {
      const top = new maptalks.Coordinate([center.x, center.y + 0.001])
      const geometry = new maptalks.LineString([
        top,
        new maptalks.Coordinate([center.x, center.y]),
        new maptalks.Coordinate([center.x + 0.002, center.y])
      ])
      layer.addGeometry(geometry)
      const topPt = map.coordinateToContainerPoint(top)
      expect(geometry.containsPoint(topPt)).to.be.ok()
      map.setPitch(60)
      expect(geometry.containsPoint(topPt)).not.to.be.ok()
      const newTopPt = map.coordinateToContainerPoint(top)
    ...
    it('circle', function () {
      const top = map.locate(center, 0, 10)
      const geometry = new maptalks.Circle(center, 10)
      layer.addGeometry(geometry)
      const topPt = map.coordinateToContainerPoint(top)
      expect(geometry.containsPoint(topPt)).to.be.ok()
      map.setPitch(60)
    ...
    it('ellipse', function () {
      const top = map.locate(center, 0, 4)
      const geometry = new maptalks.Ellipse(center, 10, 8)
      layer.addGeometry(geometry)
      const topPt = map.coordinateToContainerPoint(top)
      expect(geometry.containsPoint(topPt)).to.be.ok()
      map.setPitch(80)
    ...
    it('sector', function () {
      const top = map.locate(center, 0, 10)
      const geometry = new maptalks.Sector(center, 10, 0, 90)
      layer.addGeometry(geometry)
      const topPt = map.coordinateToContainerPoint(top)
      expect(geometry.containsPoint(topPt)).to.be.ok()
      map.setPitch(60)
