describe('GeoJSON', function () {
  ...
  describe('geojson to coordinate', function () {
    ...
    const result = maptalks.Coordinate.toCoordinates(geoJSONCoords)
    expect(result).to.have.length(geoJSONCoords.length)
    expect(result[0]).to.eql(new maptalks.Coordinate(geoJSONCoords[0]))
    const reverse = maptalks.Coordinate.toNumberArrays(result)
    expect(reverse).to.eql(geoJSONCoords)
  })

  describe('parse FeatureCollection', function () {
    const fJsons = featureCollectionGeoJSON.features
    const features = maptalks.GeoJSON.toGeometry(featureCollectionGeoJSON)
  ...
  describe('parse GeoJSON Objects', function () {
    const geometries = maptalks.GeoJSON.toGeometry(geoJSONs)
    ...
    it('evaluate MultiLineString', function () {
      const multiPolyline = geometries[5]
    ...
    it('evaluate MultiPolygon', function () {
      const multiPolygon = geometries[6]
    ...
    it('evaluate GeometryCollection', function () {
      const geometryCollection = geometries[7]
  ...
  describe('foreach call back', function () {
    it('FeatureCollection', function () {
      const fJsons = featureCollectionGeoJSON.features
      const features = maptalks.GeoJSON.toGeometry(featureCollectionGeoJSON, function (geometry) {
    ...
    it('evaluate point', function () {
      const point = maptalks.GeoJSON.toGeometry(geoJSONs[0], function (geometry) {
        geometry.config('zIndex', 0)
      })
