describe('Marker.Sprite', function () {
  it('image sprite', function (done) {
    const url = 'http://localhost:9876/resources/pattern.png'
    ...
    image.onload = function () {
      const resources = new maptalks.renderer.ResourceCache()
      resources.addResource([url], image)
      const sprite = marker._getSprite(resources)
  ...
  it('vector path marker sprite', function (done) {
    ...
    const url = maptalks.Util.getMarkerPathBase64(symbol)
    const img = new Image()
    img.onload = function () {
      const sprite = marker._getSprite()
      const canvas = sprite.canvas
  ...
  it('text marker sprite', function () {
    ...
    const sprite = marker._getSprite().canvas
