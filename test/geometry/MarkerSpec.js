describe('Geometry.Marker', function () {
  ...
  it('getCenter', function () {
    const marker = new maptalks.Marker({ x: 0, y: 0 })
  ...
  it('get2dExtent', function () {
    ...
    const extent = marker._getPainter().get2DExtent()
    expect(extent).to.be.ok()
    marker.setCoordinates(map.getCenter())
    const extent2 = marker._getPainter().get2DExtent()
  ...
  describe('symbol', function () {
    ...
    it('can be vector', function () {
      const types = ['ellipse', 'cross', 'x', 'diamond', 'bar', 'square', 'rectangle', 'triangle', 'pin', 'pie']
      ...
      for (let i = 0; i < types.length; i++) {
        ...
        const layer = new maptalks.VectorLayer('id' + i, { drawImmediate: true }).addTo(map)
        layer.addGeometry(marker)
  ...
  describe('set marker\'s Symbol', function () {
    ...
    it('unsuppored markerType', function () {
      const layer = new maptalks.VectorLayer('vector', { drawImmediate: true })
      map.addLayer(layer)
      const marker = new maptalks.Marker(map.getCenter(), {
        symbol: {
    ...
    it('change symbol in event listener', function (done) {
      const marker = new maptalks.Marker(map.getCenter())
      const layer = new maptalks.VectorLayer('vector', [marker])
  ...
  describe('events', function () {
    it('canvas events', function () {
      const vector = new maptalks.Marker(center)
  ...
  it('Marker.containsPoint', function () {
    const geometry = new maptalks.Marker(center, {
      symbol: {
        markerFile: 'images/control/2.png',
        markerHeight: 30,
        markerWidth: 22,
  ...
  it('get image marker\'s extent', function (done) {
    const geometry = new maptalks.Marker(map.getExtent().getMin().substract(1E-7, 0), {
      symbol: {
        markerFile: 'images/control/2.png'
      }
    })
  ...
  describe('function type symbols', function () {
    it('vector marker\'s size changes with zoom', function (done) {
      ...
      const layer = new maptalks.VectorLayer('id', { drawImmediate: true }).addTo(map)
      layer.addGeometry([marker])
  ...
  describe('marker rotation', function () {
    ...
    it('rotate text marker outline', function () {
      ...
      const layer = new maptalks.VectorLayer('vector', marker, { drawImmediate: true }).addTo(map)
      const outline = marker.getOutline().updateSymbol({ markerFill: '#0f0' }).addTo(layer)
  ...
  describe('rotate the geometry', function () {
    ...
    it('with a pivot', function () {
      ...
      const newCoords = marker.getCoordinates().toArray()
