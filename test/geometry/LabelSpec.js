describe('Geometry.Label', function () {
  ...
  describe('events', function () {
    it('canvas events', function () {
      const vector = new maptalks.Label('test label', center)
  ...
  describe('get/set', function () {
    it('content', function () {
      let label = '中文标签'
      const vector = new maptalks.Label(label, center)
      layer = new maptalks.VectorLayer('id')
      map.addLayer(layer)
      layer.addGeometry(vector)
  ...
  describe('alignment', function () {
    it('left', function () {
      const vector = new maptalks.Label('■■■', center, {
        textSymbol: {
          textFaceName: 'arial',
          textSize: 12,
  ...
  describe('can config', function () {
    it('configs', function () {
      const label = new maptalks.Label('label', center)
      const defaultConfig = label.config()
  ...
  it('autoSize', function () {
    ...
    const symbol = vector._getInternalSymbol()
  ...
  it('edit with special characters', function () {
    const vector = new maptalks.Label('\b\t\v\flabe\r\nl', center)
    layer = new maptalks.VectorLayer('id')
    map.addLayer(layer)
    layer.addGeometry(vector)
    vector.startEditText()
  ...
  it('edit with "Enter" characters', function () {
    const vector = new maptalks.Label('Labe\r\nl\r', center)
    layer = new maptalks.VectorLayer('id')
    map.addLayer(layer)
    layer.addGeometry(vector)
    vector.startEditText()
  ...
  describe('edit label', function () {
    ...
    it('horizontal left', function () {
      ...
      vector.startEditText()
      expect(vector.isEditingText()).to.be.ok()
      const editor = vector.getTextEditor()
    ...
    it('filter with special characters', function () {
      const vector = new maptalks.Label('\b\t\v\fLabel', center)
      layer = new maptalks.VectorLayer('id')
      map.addLayer(layer)
      layer.addGeometry(vector)
      vector.startEditText()
    ...
    it('filter with "Enter" characters', function () {
      const vector = new maptalks.Label('Label\r', center)
      layer = new maptalks.VectorLayer('id')
      map.addLayer(layer)
      layer.addGeometry(vector)
      vector.startEditText()
    ...
    it('mock input characters', function () {
      const label = new maptalks.Label('I am a Text', center)
      layer = new maptalks.VectorLayer('id')
      map.addLayer(layer)
      ...
      function startEdit () {
        ...
        const dom = label.getTextEditor().getDOM()
