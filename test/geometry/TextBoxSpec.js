describe('Geometry.TextBox', function () {
  ...
  describe('events', function () {
    it('canvas events', function () {
      const vector = new maptalks.TextBox('test label', center, 100, 40)
  ...
  describe('get/set', function () {
    it('content', function () {
      let content = '中文标签'
      const vector = new maptalks.TextBox(content, center, 100, 40)
      layer = new maptalks.VectorLayer('id')
      map.addLayer(layer)
      layer.addGeometry(vector)
  ...
  describe('can config', function () {
    it('configs', function () {
      const vector = new maptalks.TextBox('textbox', center, 100, 40)
      const defaultConfig = vector.config()
  ...
  describe('alignment', function () {
    const padding = [12, 8]
    it('left', function () {
      const vector = new maptalks.TextBox('■■■', center, 100, 100, {
        textStyle: {
          wrap: true,
          padding: padding,
          verticalAlignment: 'middle',
  ...
  it('width, height', function () {
    const vector = new maptalks.TextBox('■■■', center, 100, 50)
    const symbol = vector.getSymbol()
    expect(symbol.markerWidth).to.be.eql(100)
    expect(symbol.markerHeight).to.be.eql(50)
  })

  it('can edit', function () {
    const vector = new maptalks.TextBox('textbox', center)
    layer = new maptalks.VectorLayer('id')
    map.addLayer(layer)
    layer.addGeometry(vector)
  ...
  it('edit with special characters', function () {
    const vector = new maptalks.TextBox('\b\t\v\ftextbox', center, 100, 50)
    layer = new maptalks.VectorLayer('id')
    map.addLayer(layer)
    layer.addGeometry(vector)
  ...
  it('edit with "Enter" characters', function () {
    const vector = new maptalks.TextBox('textbox\r', center, 100, 50)
    layer = new maptalks.VectorLayer('id')
    map.addLayer(layer)
    layer.addGeometry(vector)
    vector.startEditText()
  ...
  it('clear unused text caches', function (done) {
    const cachekey = maptalks.symbolizer.TextMarkerSymbolizer.CACHE_KEY
  ...
  it('JSON of previous version\'s TextBox', function () {
    const json = { content: '岭南站/SM/A', feature: { geometry: { coordinates: [113.120816, 23.033914], type: 'Point' }, id: 'NWP_LABEL_3', type: 'Feature' }, options: { boxAutoSize: true, boxMinHeight: 30, boxMinWidth: 100, boxPadding: { height: 8, width: 15 }, draggable: true, visible: true, zIndex: 1 }, subType: 'TextBox', symbol: { markerFill: '#ffffff', markerFillOpacity: 1, markerHeight: 40, markerLineColor: '#cccccc', markerLineOpacity: 0.8, markerLineWidth: 1, markerOpacity: 0.8, markerType: 'square', markerWidth: 140, opacity: 1, textDx: 0, textDy: 0, textFaceName: 'microsoft yahei', textFill: '#000000', textHorizontalAlignment: 'middle', textLineSpacing: 1, textName: '岭南站/SM/A', textOpacity: 0.8, textSize: 18, textSpacing: 0, textVerticalAlignment: 'middle', textWrapBefore: false, textWrapCharacter: '\n' } }
    const textBox = maptalks.Geometry.fromJSON(json)
