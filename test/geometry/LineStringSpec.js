describe('Geometry.LineString', function () {
  ...
  it('getCenterInExtent', function () {
    ...
    const got = polyline.getCenterInExtent(new maptalks.Extent(20, -20, 40, 50))
  ...
  it('getCoordinates', function () {
    ...
    const polyline = new maptalks.LineString(path)
    layer.addGeometry(polyline)
  ...
  describe('creation', function () {
    it('normal constructor', function () {
      const points = [[100.0, 0.0], [101.0, 0.0], [101.0, 1.0], [100.0, 1.0], [100.0, 0.0]]
      const polyline = new maptalks.LineString(points)
  ...
  describe('rotate the geometry', function () {
    it('without a pivot', function () {
      ...
      const expected = [[10.796595235860309, 1.8092213764076168], [7.350247889316506, 11.2061475842436], [-33.81727811167417, 76.98463103926437]]
      const json = polyline.toGeoJSON().geometry.coordinates
  ...
  describe('geometry fires events', function () {
    it('events', function () {
      ...
      const vector = new maptalks.LineString(points)
  ...
  it('containsPoint', function () {
    const lineWidth = 8
    const line = new maptalks.LineString([map.getCenter(), map.getCenter().add(0.1, 0)], {
      symbol: [{
        lineWidth: lineWidth
      },
      {
    ...
    const cp = map.coordinateToContainerPoint(map.getCenter())
  ...
  it('containsPoint with arrow of point', function () {
    const lineWidth = 8
    const line = new maptalks.LineString([map.getCenter().substract(0.001, 0), map.getCenter(), map.getCenter().add(0.001, 0)], {
      enableSimplify: false,
      arrowStyle: 'classic',
  ...
  it('bug: create with dynamic textSize', function () {
    ...
    const symbol = { lineWidth: 1, lineColor: '#000', textName: '{count}', textSize: { type: 'interval', stops: [[0, 0], [16, 5], [17, 10], [18, 20], [19, 40]] } }
  ...
  it('arc curve identify', function () {
    const line = new maptalks.ArcCurve([map.getCenter(), map.getCenter().add(0.001, 0)])
    layer.addGeometry(line)
  ...
  describe('outline', function () {
    it('display outline', function () {
      ...
      const outline = line.getOutline().updateSymbol({ polygonFill: '#0f0' }).addTo(layer)
  ...
  describe('animateShow', function () {
    ...
    it('line containerExtent when drawing altitude', function () {
      ...
      const extent = polyline._getPainter().getContainerExtent().round().toString()
      console.log(extent)
