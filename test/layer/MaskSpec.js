...
describe('Spec of Masks', function () {
  ...
  function isDrawn (canvas, p) {
    ...
    const imgData = context.getImageData(p.x, p.y, 1, 1).data
    if (imgData[3] > 0) {
      return true
    }
    return false
  ...
  function testMask (layer, done) {
    const canvas = layer.getMap().getRenderer().canvas
    const c = new maptalks.Point(canvas.width / 2, canvas.height / 2)
  ...
  // test vectorlayer
  const vlayer = new maptalks.VectorLayer('v').addGeometry(new maptalks.Circle(center, 2000, {
    symbol: {
  ...
  it('#713', function (done) {
    ...
    map.addLayer(vlayer)
    const canvas = vlayer.getMap().getRenderer().canvas
    const c = new maptalks.Point(canvas.width / 2, canvas.height / 2)
