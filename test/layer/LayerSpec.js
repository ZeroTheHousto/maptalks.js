describe('Layer.Spec', function () {
  ...
  describe('id methods', function () {
    it('id will be converted to string', function () {
      const layer1 = new maptalks.TileLayer(1)
      expect(layer1.getId()).to.be.eql('1')
    })

    it('id can be null or undefined', function () {
      const layer1 = new maptalks.TileLayer(null)
    ...
    it('update id', function () {
      const layer1 = new maptalks.TileLayer('1', { renderer: 'canvas' })
      map.addLayer(layer1)
      layer1.setId('2')
    ...
    it('prevent loading in onLoad', function (done) {
      const layer = new maptalks.VectorLayer('1', { renderer: 'canvas' })
      let ready = false
    ...
    it('onAdd is called when added to map', function (done) {
      const layer = new maptalks.VectorLayer('1')
  ...
  describe('zindex of layers', function () {
    it('default zindex is 0', function () {
      ...
      const layer3 = new maptalks.VectorLayer('3')
    ...
    it('setZIndex in options before adding to map to bring a layer to front', function () {
      const layer1 = new maptalks.TileLayer('1', { renderer: 'canvas', zIndex: 1 })
      const layer2 = new maptalks.VectorLayer('2')
      const layer3 = new maptalks.VectorLayer('3')
  ...
  describe('change order of layers', function () {
    ...
    it('sort layers by map and paint', function (done) {
      const layer1 = new maptalks.VectorLayer('1', new maptalks.Circle(map.getCenter(), 100, {
        symbol: {
          polygonFill: '#f00'
        }
      }))
      const layer2 = new maptalks.VectorLayer('2', new maptalks.Circle(map.getCenter(), 100, {
        symbol: {
          polygonFill: '#0f0'
        }
      }))
      const layer3 = new maptalks.VectorLayer('3', new maptalks.Circle(map.getCenter(), 100, {
        symbol: {
          polygonFill: '#00f'
        }
      }))
  ...
  describe('set a mask of a vector marker', function () {
    ...
    it('to a tile layer', function () {
      const tilelayer = new maptalks.TileLayer('tile with mask', {
    ...
    it('to a VectorLayer', function () {
      const vectorlayer = new maptalks.VectorLayer('vector with mask')
  ...
  it('change opacity', function (done) {
    const layer1 = new maptalks.TileLayer('1', { renderer: 'canvas' })
    const layer2 = new maptalks.VectorLayer('2', new maptalks.Marker(map.getCenter(), {
      symbol: {
        markerType: 'ellipse',
        markerWidth: 10,
        markerHeight: 10,
    ...
    layer2.once('layerload', function () {
      layer2.once('layerload', function () {
        ...
        const size = map.getSize().toPoint()
        const context = canvas.getContext('2d')
        const imgData = context.getImageData(Math.round(size.x / 2), Math.round(size.y / 2), 1, 1).data
  ...
  it('#getCollisionIndex', function () {
    const layer = new maptalks.TileLayer('1', { renderer: 'canvas' })
