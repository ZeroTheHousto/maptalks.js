describe('VectorLayer.Spec', function () {
  ...
  describe('creation', function () {
    ...
    it('create with geometries', function () {
      const layer = new maptalks.VectorLayer('v', { cursor: 'default' })
      expect(layer.getCount()).to.be.eql(0)
      expect(layer.options.cursor).to.be.eql('default')
    })

    it('create with geometries', function () {
      const layer = new maptalks.VectorLayer('v', [new maptalks.Marker(map.getCenter()), new maptalks.Marker(map.getCenter())], { cursor: 'default' })
    ...
    it('create on a given canvas', function (done) {
      ...
      const layer = new maptalks.VectorLayer('v', [new maptalks.Marker(map.getCenter(), { symbol: symbol })], { canvas: canvas })
      layer.once('layerload', function () {
        expect(layer).to.be.painted(0, 0)
        const w = canvas.width; const h = canvas.height
        const color = canvas.getContext('2d').getImageData(w / 2, h / 2, 1, 1).data
  ...
  describe('can addGeometry', function () {
    ...
    it('add a geometry with id of 0', function () {
      ...
      const geo = layer.getGeometryById(0)
      expect(geo).to.be.ok()
    })
  ...
  describe('paint geometry', function () {
    ...
    it('paint a geometry', function (done) {
      const circle = new maptalks.Circle(map.getCenter(), 100, {
        symbol: {
          polygonFill: '#f00'
        }
      })
    ...
    it('repaint when map resized', function (done) {
      ...
      layer.once('layerload', function () {
        let sx = 1; let sy = 1
        const scaleFn = layer.getRenderer().context.scale
    ...
    it('change zindex', function (done) {
      const circle1 = new maptalks.Circle(map.getCenter(), 100, {
        symbol: {
          polygonFill: '#f00'
        }
      })
      const circle2 = new maptalks.Circle(map.getCenter(), 100, {
        symbol: {
          polygonFill: '#0f0'
        }
      })
    ...
    it('ignore geometries out of container extent', function (done) {
      const ne = map.getSize().toPoint().multi(1 / 2)
      const circle = new maptalks.Circle(map.getExtent().getMax(), 10, {
        symbol: {
          polygonFill: '#f00'
        }
      })
  ...
  describe('can setStyle', function () {
    ...
    it('symbol first', function () {
      ...
      const vectors = new maptalks.VectorLayer('symbol-style', { drawImmediate: true })
        .setStyle({
          filter: ['==', '$type', 'Point'],
          symbol: styleSymbol
        })
